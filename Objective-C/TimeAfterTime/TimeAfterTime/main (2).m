...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        //Программа, которая определяет, действует ли сейчас летнее время
        
        NSTimeZone *summerTime = [NSTimeZone systemTimeZone];
        NSLog(@"Часовой пояс в моем городе %@", summerTime);
        if ([summerTime isDaylightSavingTime] == YES)
            NSLog(@"В данный момент действует летнее время");
        else
            NSLog(@"Летнее время в данный момент не действует");
